...
exports.retreiveTypeCategory = (req, res, next) => {
  const catType = req.params.typeCat;
...
exports.addCategory = (req, res, next) => {
  ...
  const insertCat = new categorySchema({
  ...
  insertCat.save().then(() => res.status(200).send({ catMsg: 'Category details Inserted Successfully!!!' }));
};
...
exports.updatingCategory = (req, res, next) => {
  ...
  const findingAndUpdatingCat = categorySchema.findOneAndUpdate({ _id: id }, {
    type_of_category: categoryType.trim(),
    category_name: categoryName.trim(),
    qoute: qoute.trim(),
  });
...
exports.deletingCategory = (req, res, next) => {
  ...
  const { id } = req.params;
  const findingAndDeletingCat = categorySchema.findOneAndDelete({ _id: id });
