...
exports.addDelivery = (req, res, next) => {
  const {
    userID, productID, userName, productName, finalPrice, productLink, points,
  } = req.body;
  const userMongooseId = new mongoose.Types.ObjectId(userID);
  const productMongooseId = new mongoose.Types.ObjectId(productID);
  console.log(req.body);
  console.log(userMongooseId);

  const insertDelivery = new deliverySchema({
  ...
  insertDelivery.save().then(() => res.status(200).send({ deliverMsg: 'Delivery details Inserted Successfully!!!' }));
};
...
exports.updatingDelivery = (req, res, next) => {
  ...
  const userMongooseId = userID ? new mongoose.Types.ObjectId(userID) : '';
  const productMongooseId = productID ? new mongoose.Types.ObjectId(productID) : '';
  const { id } = req.params;
  console.log(req.params);
  // console.log(req.body);
  deliverySchema.findById(id).then((data) => {
    const findingAndUpdatingDelivery = deliverySchema.findOneAndUpdate({ _id: id }, {
...
exports.deletingDelivery = (req, res, next) => {
  ...
  const { id } = req.params;
  const findingAndDeletingDelivery = deliverySchema.findOneAndDelete({ _id: id });
