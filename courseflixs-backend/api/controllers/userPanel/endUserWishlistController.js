...
exports.addWishlist = (req, res, next) => {
    ...
    wishlistSchema.findOne({ $and: [{ userID: userID }, { proID: proID }] }).then((result) => {
        if (result) {
            return res.status(200).send({ wiMsg: "Product already exist into the wishlist!" });

        } else {
            var insertWishlist = new wishlistSchema({
                proID: new mongoose.Types.ObjectId(proID),
                userID: new mongoose.Types.ObjectId(userID),
            });
            console.log("request reached")

            insertWishlist.save().then(() => {
                return res.status(200).send({ wishMsg: "Product added into the wishlist!" });
            });
...
exports.deleteWishlist = (req, res, next) => {
    ...
    var findingAndDeletingWishlist = wishlistSchema.findOneAndDelete({_id:wishID});
